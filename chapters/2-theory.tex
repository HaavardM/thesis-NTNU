...
\textit{\acrfull{pgm}}, often called Bayesian Networks, restricts the number of conditional distributions by assuming conditional independence, as defined in \cref{eq:conditional_independence}, between variables. \acrshort{pgm}'s are graphs, where nodes represent random variables and edges represents statistical dependence between the variables. 
...
\acrshort{dgm} allows for straight-forward factorizations of the joint-distribution 
...
\acrshort{mrf}'s are undirected graphical models, which do not require the specification of edge direction. They are in some domains, such as relational or spatial data , more natural than \acrshort{dgm}'s as they are symmetric, i.e. the information flows both ways \cite{murphy}. The \acrshort{mrf} in \cref{fig:mrf} shows that $B$ and $C$ is only independent if both $A$ and $D$ is observed, i.e. $B \perp C | \; A, D$. The important thing to notice is that the \acrshort{mrf} makes vastly different assumptions than the \acrshort{dgm}.
...
\end{equation}
where is $\mathcal{C}$ is the set of all \textit{maximal cliques}, i.e. largest, fully connected subgraphs. All variables in a maximal clique is therefore dependent on eachother. $\psi_c(\cdot)$ is the \textit{potential function} for the nodes in the clique $c$, and determines the strength of the interaction between the variables $\bf{x}_c$. The \textit{partition function}
\begin{equation}
    Z = \int_\mathbf{x} \prod_{c \in \mathcal{C}} \psi_c(\mathbf{x}_c)
\end{equation}
...
\acrshort{mrf}'s are a generalization of \acrshort{dgm}'s with very specific potential functions that corresponds to conditional distributions and implies a DAG structure. \acrshort{mrf}'s are therefore more powerfull than \acrshort{dgm}, but are also more computationally challenging. 

\acrshort{mrf}'s is not the subject of this paper, but some derivations in later chapters requires some familiarity.
...
\label{fig:dgm}
\end{subfigure}
\caption{\acrshort{pgm} representations of a probabilistic model with $4$ variables.}
\end{figure}
