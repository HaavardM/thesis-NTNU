...
\subsection{Softmax}
The softmax is a useful transformation from $\mathcal{R}^N$ to $\{\mathbf{x} \in (0,1)^N | \sum x_i = 1\}$ \cite{murphy, tensorflow2015}. It can be viewed as a multivariate generalization of the sigmoid function. 
...
\end{equation}
All the continious distributions mentioned so far are part of the Exponential Family. Distributions in the exponential family always have conjugate priors, however they may not be standard, well-known distributions \cite{murphy}. 
