\documentclass[american]{ntnuthesis}

\title{Gaussian Processes for long-term trajectory prediction using historical AIS data }
\shorttitle{Gaussian Process AIS Trajectory Predction}
\author{%
Håvard Skåra Mellbye
\vspace{1cm}\\
Supervisor: Edmund Førland Brekke \\ 
Co-supervisor: Trym Tengesdal
}
\shortauthor{Mellbye}
\date{}


\addbibresource{thesis.bib}

\usepackage{tikz}
\usetikzlibrary{bayesnet}
\usepackage{ifthen}
\usepackage[many]{tcolorbox}
\usepackage{todonotes}
\definecolor{bleu-de-france}{cmyk}{0.79, 0.39, 0.00, 0.09}
\definecolor{bleu-de-france-light}{cmyk}{0.11, 0.04, 0.00, 0.01}



\input{glossary.tex} % add glossary and acronym lists before document

\begin{document}
\tcbset{colback=bleu-de-france-light, colframe=bleu-de-france}
% gives the width of the current document in pts
\input{chapters/0-preface.tex}
\input{chapters/0a-abstract.tex}
\input{chapters/0b-sammendrag.tex}

\tableofcontents
\listoffigures
\listoftables
%\lstlistoflistings


\printglossary[type=\acronymtype] % Print acronyms
\printglossary                    % Print glossary

\input{chapters/1-introduction.tex}
\input{chapters/2-prior-work.tex}
\input{chapters/3-theory.tex}
\input{chapters/4-ais.tex}
\input{chapters/5a-gp-trajectories}
\input{chapters/5b-gp-trajectories}
\input{chapters/6-implementation-and-results}
\input{chapters/7-discussion.tex}
\input{chapters/8-conclusion.tex}

\chapter*{\bibname}
\printbibliography[heading=none]

\appendix
\input{appendices/a-appendix.tex}

\end{document}
