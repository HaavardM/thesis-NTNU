...
\begin{algorithm}[h]
    \begin{algorithmic}[1]
        \Procedure{GP-EKF-SL}{$\hat{\boldsymbol{x}}_{t}$, $\hat{\boldsymbol{P}}_{t}$, | $\mathbf{R}$, $r$}
        \State $\mathcal{Y}_t = \mathcal{Y}_t(\boldsymbol{x}_t) = \{\boldsymbol{z}_i \in \mathcal{D} : \; ||\boldsymbol{x}_t - \boldsymbol{z}_i|| \leq r\}$\Comment{Relevant measurements}
        \State $S_t = \frac{1}{N} \sum_{\boldsymbol{z}_i \in \mathcal{Y}_t} \boldsymbol{z}_i$\Comment{Compute summary}
            \State $\boldsymbol{W}_t = \hat{\boldsymbol{P}}_t (\hat{\boldsymbol{P}}_t + \frac{1}{N} \boldsymbol{R})^{-1}$ \Comment{Kalman Gain}
            \State $\boldsymbol{x}_t = \hat{\boldsymbol{x}}_t + \boldsymbol{W}(S_t - \hat{\boldsymbol{x}}_t)$\Comment{Conditional Mean}

            \State $\boldsymbol{P}_t = (\boldsymbol{I} - \boldsymbol{W}_t) \hat{\boldsymbol{P}}_t$\Comment{Conditional Variance}
