...
The \acrshort{sl} update above end up using an overly simplistic measurement model, and do not consider the possibility that some measurements may originate from different underlying trajectories. For the target vessel following a specific underlying trajectory, the dataset may actually contain a substantial false measurements which negatively affect the prediction. As the \acrshort{sl} update has no notion of false measurements, the result is overconfident uncertianty measurements can be seen in \cref{fig:gp_ekf_with_syn}.
