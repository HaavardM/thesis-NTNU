...
The problem is that the dataset $\mathcal{D}$ does not contain any samples from the target vessel's future trajectory, though it contains a lot of data from trajectories that are assumed to be similar. The challenge is to relate the target vessel's trajectory to the data-generating process for the dataset $\mathcal{D}$.
...
The first approach makes the assumption that if the vessel is indeed following a common trajectory, there should be plenty of historical \acrshort{ais} messages distributed in the vicinity of any given state in the trajectory $\boldsymbol{x}_t$.
