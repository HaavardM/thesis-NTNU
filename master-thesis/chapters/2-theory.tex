...
\Input{$X_*$ (prediction inputs), $\boldsymbol{y}$ (observed values), $k$ (kernel function), $X$ (observed inputs)}

$L = cholesky\big(K(X, X) + \sigma I\big)$\;
$\boldsymbol{\alpha} = L^\intercal \backslash (L \backslash \boldsymbol{y})$\;
$\boldsymbol{v} = L \backslash K(X, X_*)$\;
$\bar{\boldsymbol{f}}_* = m(X_*) + K(X_*, X) \boldsymbol{\alpha}$\;
$\mathbb{V}[\boldsymbol{f}_*] = K(X_*, X_*) - \boldsymbol{v}^\intercal \boldsymbol{v}$\;
\KwResult{$\bar{\boldsymbol{f}}$ (mean), $\mathbb{V}[\boldsymbol{f}_*]$ (covariance)}
